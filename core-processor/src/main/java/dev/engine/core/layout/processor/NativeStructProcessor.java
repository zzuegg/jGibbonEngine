 ...
 */
@SupportedAnnotationTypes({"dev.engine.core.layout.NativeStruct", "dev.engine.core.Discoverable"})
...
public class NativeStructProcessor extends AbstractProcessor {
    ...
    private static final String DISCOVERABLE_ANNOTATION = "dev.engine.core.Discoverable";

    /** All discoverable classes seen across rounds (qualified name → has NativeStruct companion). */
    private final java.util.LinkedHashMap<String, Boolean> discoverableClasses = new java.util.LinkedHashMap<>();
    ...
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        for (var annotation : annotations) {
            ...
            for (var element : roundEnv.getElementsAnnotatedWith(annotation)) {
                if (NATIVE_STRUCT_ANNOTATION.equals(annotationName)) {
                    ...
                    discoverableClasses.put(((TypeElement) element).getQualifiedName().toString(), true);
                } else if (DISCOVERABLE_ANNOTATION.equals(annotationName)) {
                    if (element instanceof TypeElement typeElement
                            && typeElement.getKind() != ElementKind.ANNOTATION_TYPE) {
                        // Don't double-count @NativeStruct records (they're also @Discoverable via meta-annotation)
                        discoverableClasses.putIfAbsent(typeElement.getQualifiedName().toString(), false);
                    }
                }
            }
        }

        // In the final round, generate the aggregate DiscoveryRegistry
        if (roundEnv.processingOver() && !discoverableClasses.isEmpty()) {
            generateDiscoveryRegistry();
        }
    ...
    private void generateDiscoveryRegistry() {
        ...
        try {
            ...
            var file = processingEnv.getFiler().createSourceFile(fqn);
            try (var w = file.openWriter()) {
                if (!pkg.isEmpty()) {
                    w.write("package " + pkg + ";\n\n");
                }
                w.write("import dev.engine.core.DiscoveryRegistry;\n");
                w.write("import java.util.List;\n\n");
                w.write("/**\n");
                w.write(" * Generated by annotation processor. Lists all {@code @Discoverable}\n");
                w.write(" * classes in this compilation unit and initializes their companions.\n");
                w.write(" */\n");
                w.write("public final class " + registryClassName + " implements DiscoveryRegistry {\n\n");
                ...
                // initialize() method
                w.write("    @Override\n");
                w.write("    public void initialize() {\n");
                for (var entry : discoverableClasses.entrySet()) {
                    if (entry.getValue()) {
                        // Has NativeStruct companion — call init()
                        w.write("        " + entry.getKey() + "_NativeStruct.init();\n");
                    }
                }
                w.write("    }\n");
                w.write("}\n");
            ...
            // Write resource file so Discovery can find this registry at runtime.
            // Uses the same path as ServiceLoader but loaded manually to work
            // across platforms (TeaVM doesn't support ServiceLoader).
            var resource = processingEnv.getFiler().createResource(
                    javax.tools.StandardLocation.CLASS_OUTPUT, "",
                    "META-INF/services/dev.engine.core.DiscoveryRegistry");
            try (var rw = resource.openWriter()) {
                rw.write(fqn + "\n");
            }

        } catch (IOException e) {
            processingEnv.getMessager().printMessage(Diagnostic.Kind.ERROR,
                    "Failed to generate DiscoveryRegistry: " + e.getMessage());
        }
    }
